...
TempNet<-function(file,ftype="fasta",ages=NA,mut_size=1.5,nohap_size=.5,layernm=NA,mut.len=1,color=NA,
	invert=F,planes=F,vcol=F,theta=0,phi=pi/6,hapid=F,mergelayer=list(),confirm=T,save=T,load=F) { #v1.8
    ...
    for(i in 1:nrow(s)) for(j in lvls) lvl[i,j]=sum(ages[attr(s,"index")[[i]]]==j)
    ...
    if(load & file.exists(fnm)) elip=read.table(fnm)
        ...
        for(i in 1:nrow(net)) {
            ...
            d=sum(sqrt(lvl[newl,bigcircs]/pi))+net[i,3]*mut.len #dist b/wn circle centers
    		elip[newl[2],1:2]=elip[newl[1],1:2]+d*c(cos(elip[newl[1],3]),sin(elip[newl[1],3]))
            elip[newl[2],3]=elip[newl[1],3]+pi
            elip[newl,3]=(elip[newl,3]+2*pi/links[newl])%%(2*pi)
        }
    }
    ...
    while(clk[1]!=0) {
        ...
        for(i in 1:ncol(lvl)) {
			...
			for(j in rev(order(elip[,2]))) #vertical lines
    			if(i<ncol(lvl)) if(lvl[j,i+1]>0 && lvl[j,i]>0) #exists at adjacent time points
            ...
            for(j in rev(order(elip[,2]))) if(lvl[j,i]>0) in3d(cbind(elip[j,1],elip[j,2],h*(i-1)),theta,"text",phi,labels=ifelse(hapid,j,lvl[j,i])) #n in haplogroup
